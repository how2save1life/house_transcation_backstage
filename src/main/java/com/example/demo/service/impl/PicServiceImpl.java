...
public class PicServiceImpl implements PicService {
    ...
    public String saveHousePic(MultipartFile file, String houseId) {
        ...
        String fileName;//文件名
        //判断用户登录类型是否正确
        if (houseRepository.findByHouseId(houseId)!=null) {
            fileName = file.getOriginalFilename();
            System.out.println("上传的文件原名称:" + fileName);
            type = fileName.contains(".") ? fileName.substring(fileName.lastIndexOf(".")) : null;//.后缀
            //文件类型是否为空
            if (type != null) {
                //是否是jpg、png
                if (".PNG".equals(type.toUpperCase()) || ".JPG".equals(type.toUpperCase())) {
                    fileName = "1" + type;//文件名 id+后缀名
                    path = "D:/vue/housing_transaction_management/static/pics/house/" + houseId + "/" + fileName;//路径
                    savePath = "/pics/house/"+ houseId + "/" + fileName;//存入数据库的 部分路径
                    try {
                        File temp = new File(path);
                        //
                        isPath(temp);//若路径不存在则创建
                        //
                        file.transferTo(temp);//存图
                        houseRepository.updateHousePic(houseId,savePath);
                    } catch (IOException e) {
                        e.printStackTrace();
                        return "error";//异常
                    }
